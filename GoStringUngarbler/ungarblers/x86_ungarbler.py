...
class GoStringUngarblerX86(GoStringUngarbler):
    """
    Class for the x86 string ungarbler engine
    
    Attributes
    ----------
    skip_prologue: bool 
        
        Skipping prologue of the currently executing subroutine
    """
    skip_prologue: int = False
    ...
    def __init__(self, lief_binary: lief.Binary, binary_data: bytes):
        ...
        self.unicorn_emu = Uc(UC_ARCH_X86, UC_MODE_32)
    ...
    def reset_stack_and_heap(self):
        ...
        self.unicorn_emu.reg_write(UC_X86_REG_ESP, self.stack_base + self.stack_size // 2)
        self.unicorn_emu.reg_write(UC_X86_REG_EBP, self.stack_base + self.stack_size // 2)
    ...
    def instruction_hook_seed(self, uc, address, size, user_data) -> None:
        ...
        if self.skip_prologue:
            self.skip_prologue = False
            # skip prologue length
            self.unicorn_emu.reg_write(UC_X86_REG_EIP, instruction.address + 0x14)
            
        if instruction.mnemonic == 'call':
            if self.runtime_newobject_call_count < 4:
                heap_allocated_mem = self.heap_alloc(0x100)
                # return allocated buffer to ESP + 4
                esp = self.unicorn_emu.reg_read(UC_X86_REG_ESP)
                heap_allocated_mem = struct.pack('<I', heap_allocated_mem)
                self.unicorn_emu.mem_write(esp + 4, heap_allocated_mem)
                # skip all 4 calls. We just manually do a heap alloc for each
                self.unicorn_emu.reg_write(UC_X86_REG_EIP, instruction.address + 5)
                ...
                if instruction.op_str[0] != 'e':
                    # call    runtime_growslice 
                    # should just return the pointer to the old buffer
                    # 0:  8b 44 24 1c             mov    eax,DWORD PTR [esp+0x1c]
                    # new length: [esp + 4]
                    # need to write to new length: [0x18]
                    # need to write to new cap: [0x18] + 0x10
                    esp = self.unicorn_emu.reg_read(UC_X86_REG_ESP)
                    
                    # new length from argument
                    new_length = self.unicorn_emu.mem_read(esp + 4, 4)
                    
                    # skip call
                    self.unicorn_emu.reg_write(UC_X86_REG_EIP, instruction.address + 5)
                    
                    heap_grow_mem = struct.pack('<I', self.heap_base + self.heap_alloc_offset)
                    self.unicorn_emu.mem_write(esp + 0x14, heap_grow_mem)
                    
                    # write new length
                    self.unicorn_emu.mem_write(esp + 0x18, bytes(new_length))
                    
                    # new cap
                    self.unicorn_emu.mem_write(esp + 0x1C, bytes(new_length))
                else:
                    # call registers
                    self.skip_prologue = True
    ...
    def emulate(self, func: Function) -> str:
        ...
        esp = self.unicorn_emu.reg_read(X86_REG_ESP)
        
        decrypted_str_ptr = struct.unpack('<I', self.unicorn_emu.mem_read(esp + 0x4, 4))[0]
        decrypted_str_size = struct.unpack('<I', self.unicorn_emu.mem_read(esp + 0x8, 4))[0]
